<template lang="html">
  <div :val="value_">
    ...
    <div>
      ...
      <el-input-number v-model="cycle.start" :max="31" size="mini" style="width: 100px;" :min="1" @change="type = '2'" />
      <span style="margin-left: 5px; margin-right: 5px;">至</span>
      <el-input-number v-model="cycle.end" style="width: 100px;" :min="2" :max="31" size="mini" @change="type = '2'" />
      日
    </div>
    <div>
      <el-radio v-model="type" label="3" size="mini" border>循环</el-radio>
      <span style="margin-left: 10px; margin-right: 5px;">从</span>
      <el-input-number v-model="loop.start" :min="1" size="mini" :max="31" style="width: 100px;" @change="type = '3'" />
      ...
      <el-checkbox-group v-model="appoint">
        <div v-for="i in 4" :key="i" style="margin-left: 10px;  line-height: 25px;">
          <template v-for="j in 10">
            <el-checkbox v-if="parseInt((i - 1) + '' + (j - 1)) < 32 && !(i === 1 && j === 1)" :key="j" :label="(i - 1) + '' + (j - 1)" @change="type = '4'" />
          </template>
        </div>
      </el-checkbox-group>
    </div>
